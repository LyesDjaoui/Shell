...
pub fn execute_command(args: &[String] , history: &[String]) -> Option<String> {
    ...
    if has_pipe {
        return commands::handle_pipe(&clean_args , stdout_path , stderr_path , stdout_append , stderr_append);
    }
    ...
    match clean_args.as_slice() {
        ...
        [cmd, rest @ ..] if cmd == "echo" || cmd == "type" => {
            ...
            if let Some(path) = &stdout_path {
                match utils::get_file_writer(path, stdout_append) {
                    Ok(mut file) => {
